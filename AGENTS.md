...
- E2E, triggered by a diff in any non-test `.go` file under `internal/userattachments/`: run it on the pull request. Every implementation file in that package participates in the upload or auth path, directly or through the state and API clients those paths depend on, so the trigger is the package rather than a filename list that drifts as files are added. Over-triggering costs one run; under-triggering ships unverified upload behavior. Confirm `auth status` valid, `upload` real PNG and MP4, verify canonical `user-attachments` URLs plus GitHub inline rendering (inline image and `<video` in rendered HTML). Record target, digests, URLs as evidence in the pull request.
