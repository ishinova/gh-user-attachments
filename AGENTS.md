...
- E2E, triggered by a diff in any non-test `.go` file in the repository: run it on the pull request. Every Go file ships inside the binary and reaches the upload or auth path, directly or through the entry point, state, and API clients those paths depend on. The trigger is the whole Go source rather than a directory or filename list, because any narrower rule drifts as files are added. Over-triggering costs one run; under-triggering ships unverified upload behavior. Confirm `auth status` valid, `upload` real PNG and MP4, verify canonical `user-attachments` URLs plus GitHub inline rendering (inline image and `<video` in rendered HTML). Record target, digests, URLs as evidence in the pull request.
