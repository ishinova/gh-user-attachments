...
- Skill contract, triggered by a diff in any non-test `.go` file or under `skills/gh-user-attachments/`: update `skills/gh-user-attachments/` in same pull request so `SKILL.md` and `agents/openai.yaml` match the surface, or state in the body why the Skill already matches. The usual evidence is that regenerating the goldens moved nothing; a pull request that introduces or restructures a golden has no such baseline, so it compares the Skill against the recorded surface directly and says what it checked. The trigger is the whole Go source because a change to what a caller observes can originate anywhere in it, and no narrower rule stays true as the code moves. Over-triggering costs one regeneration and one sentence; under-triggering ships a deployed Skill that disagrees with the released CLI. Keep deployed Skill unchanged until the release is published and verified.
